...
public class PantallaPelicula extends JPanel {
        ...
        public PeliculaCard(int peliculaId, String titulo, String urlImagen, String clasificacionEdad) {
            ...
            addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    parentPanel.mostrarPantallaPelicula(peliculaId);
                }
            });
        }
    ...
    public PantallaPelicula(int idPelicula, M4Panel parentPanel) {
        ...
        posterContainer.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (trailerUrl != null && !trailerUrl.isEmpty() && 
                    ...
                    try {
                        VideoPlayerDialog.openVideo((Frame) SwingUtilities.getWindowAncestor(PantallaPelicula.this), trailerUrl);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                        new CustomDialog((Frame) SwingUtilities.getWindowAncestor(PantallaPelicula.this), "Error al abrir el reproductor: " + ex.getMessage()).setVisible(true);
                    }
                } else {
                    new CustomDialog((Frame) SwingUtilities.getWindowAncestor(PantallaPelicula.this), "Tráiler no disponible").setVisible(true);
                }
            }
        });
    ...
    private JPanel crearPanelDerecho() {
        ...
        btnComprar.addActionListener(e -> {
            if (cbIdioma.getSelectedIndex() == 0) {
                CustomDialog dialog = new CustomDialog((Frame) SwingUtilities.getWindowAncestor(PantallaPelicula.this), "Por favor, seleccione un idioma.");
                dialog.setVisible(true);
                return;
            }
            if (cbFormato.getSelectedIndex() == 0) {
                CustomDialog dialog = new CustomDialog((Frame) SwingUtilities.getWindowAncestor(PantallaPelicula.this), "Por favor, seleccione un formato.");
                dialog.setVisible(true);
                return;
            }
            ...
            if (seleccionada != null) {
                ...
                CustomDialog dialog = new CustomDialog((Frame) SwingUtilities.getWindowAncestor(PantallaPelicula.this), "Por favor, seleccione un horario.");
                dialog.setVisible(true);
            }
        });
    ...
    private void actualizarFiltros(java.util.Date fecha) {
        new SwingWorker<Map<String, java.util.List<String>>, Void>() {
            ...
            protected Map<String, java.util.List<String>> doInBackground() throws Exception {
                ...
                Connection conn2 = M4Panel.getConexion();
    ...
    private void abrirPantallaButacas(FuncionInfo funcion) {
        parentPanel.mostrarPantallaButacas(funcion.idFuncion, funcion.idSala, this.idPelicula);
    }
}
